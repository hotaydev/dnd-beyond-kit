...
async function rollDice(dice) {
  ...
  const expandBtn = document.querySelector('div[data-cy-pod-toggler=true]');
  if (!expandBtn.classList.contains('isOpen')) {
    expandBtn.click();
  }

  await sleep(100);
  const chatBtn = document.querySelector('div[data-cy-pod-icon-type="chat"]');
  if (chatBtn.attributes.getNamedItem('aria-selected').value === "false") {
    chatBtn.click();
  }

  await sleep(100);
  const msgInput = document.querySelector('div[data-cy-chat-input-container="true"] input');
